    ...
    def seek(self, offset, *args):
        ...
        if whence == 0:
            if not isvalidpos(offset):
                printh("invalid seek: from %x to SET:%x" % (self.curpos, offset))
...
class BTree(object):
    ...
    def __init__(self, fh):
        ...
        if data[13:].startswith(b"B-tree v 1.5 (C) Pol 1990"):
            ...
            printh("unknown btree: %s" % hexdump(data))
    ...
    def dump(self):
        ...
        printh("pagesize=%08x, reccount=%08x, pagecount=%08x" % (self.pagesize, self.reccount, self.pagecount))
        self.dumpfree()
    ...
    def dumpfree(self):
        ...
        pn = self.firstfree
        if pn == 0:
            printh("no free pages")
            return
        while pn:
            self.fh.seek(pn * self.pagesize)
            data = self.fh.read(self.pagesize)
            if len(data) == 0:
                printh("could not read FREE data at page %06x" % pn)
                break
            ...
            for pn in freepages:
                self.fh.seek(pn * self.pagesize)
                data = self.fh.read(self.pagesize)
                printh("%06x: free: %s" % (pn, hexdump(data[:64])))
            pn = nextfree
    ...
    def dumptree(self, pn):
        ...
        """
        page = self.readpage(pn)
        printh("%06x: preceeding = %06x, reccount = %04x" % (pn, page.preceeding, page.count))
        ...
        while True:
            try:
                ...
                if len(pagedata) == 0:
                    ...
                    printh("%06x: incomplete - %d bytes ( pagesize = %d )" % (pn, len(pagedata), self.pagesize))
                    break
                ...
                printh("%06x: ERROR decoding as B-tree page: %s" % (pn, e))
            pn += 1
...
class ID0File(object):
    ...
    def __init__(self, idb, fh):
        ...
        if self.wordsize not in (4, 8):
            printh("Can not determine wordsize for database - assuming 32 bit")
            self.wordsize = 4
    ...
    def int(self, *args):
        ...
        if data is not None:
            ...
            printh("can't get int from %s" % hexdump(data))
...
class ID1File(object):
    ...
    def __init__(self, idb, fh):
        ...
        if magic in (b'Va4\x00', b'Va3\x00', b'Va2\x00', b'Va1\x00', b'Va0\x00'):
            ...
            if always3 != 3:
                printh("ID1: first dword != 3: %08x" % always3)
            if always2k != 0x800:
                printh("ID1: third dword != 2k: %08x" % always2k)
            seglistofs = 20
            seginfosize = 2
        else:
    ...
    def dump(self):
        ...
        for seg in self.seglist:
            printh("==== %08x-%08x" % (seg.startea, seg.endea))
            if seg.endea - seg.startea < 30:
                for ea in range(seg.startea, seg.endea):
                    printh("    %08x: %08x" % (ea, self.getFlags(ea)))
            else:
...
class NAMFile(object):
    ...
    def __init__(self, idb, fh):
        ...
        if magic in (b'Va4\x00', b'Va3\x00', b'Va2\x00', b'Va1\x00', b'Va0\x00'):
            ...
            if always1 != 1: printh("nam: first hw = %d" % always1)
            if always0 != 0: printh("nam: third dw = %d" % always0)
        elif magic == b'VA*\x00':
            always3, always1, always2k, npages, always0, nnames = struct.unpack_from("<LLLL" + fmt + "L", hdrdata, 4)
            if always3 != 3: printh("nam: 3 hw = %d" % always3)
            if always1 != 1: printh("nam: 1 hw = %d" % always1)
            if always0 != 0: printh("nam: 0 dw = %d" % always0)
            if always2k != 0x800: printh("nam: 2k dw = %d" % always2k)
            pagesize = 0x2000
        else:
    ...
    def dump(self):
        printh("nam: nnames=%d, npages=%d, pagesize=%08x" % (self.nnames, self.npages, self.pagesize))
