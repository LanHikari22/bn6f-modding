...
from pyidbutil.idblib import printh
...
def licensestring(lic):
    ...
    if not lic:
        return
    if len(lic) != 127:
        printh("unknown license format: %s" % hexdump(lic))
        return
...
def dumpuser(id0):
    ...
    if orignode:
        ...
        printh("orig: %s" % licensestring(user0))
    curnode = id0.nodeByName('$ user1')
    if curnode:
        user1 = id0.bytes(curnode, 'S', 0)
        printh("user: %s" % licensestring(user1))
...
def dumpinfo(id0):
    ...
    if ldr:
        printh("loader: %s %s" % (id0.string(ldr, 'S', 0), id0.string(ldr, 'S', 1)))

    root = id0.nodeByName("Root Node")
    if not root:
        printh("database has no RootNode")
        return
    ...
    if params:
        ...
        cpu = strz(cpu, 0)
        printh("cpu: %s, version=%d, filetype=%s, ostype=%s, apptype=%s, core:%x, size:%x" % (cpu, version, ftstring(filetype), osstring(ostype), appstring(apptype), corestart, coresize))
    ...
    printh("idaver=%s: %s" % (nonefmt("%04d", idaver), idaver2))
    ...
    printh("nopens=%s, ctime=%s, crc=%s, md5=%s" % (nonefmt("%d", nopens), nonefmt("%08x", ctime), nonefmt("%08x", crc), hexdump(srcmd5) if srcmd5 else "-"))
...
def dumpstructmember(id0, spec):
    ...
    if id0.wordsize == 8:
        ...
        f = spec
    printh("     %02x %02x %08x %02x: " % tuple(f[1:]), end="")
    ...
    printh("%-40s" % name, end="")
    if enumid:
        printh(" enum %08x" % enumid, end="")
    if struct:
        printh(" struct %08x" % struct, end="")
    if ptrseg:
        # packed
        ...
        printh(" ptr %s" % ptrseg, end="")
    if eltype:
        printh(" type %s" % eltype, end="")
    printh()
...
def dumpstruct(id0, node):
    ...
    try:
        ...
        printh("struct '%s' has corrupted M (data) node: %s" % (name, binascii.b2a_hex(packed)))
        return
    ...
    # note: 64bit nrs are stored low32, high32
    printh("struct %s, 0x%x%s" % (name, spec[0], extra))
    #  spec[0] = flags
    ...
    if len(spec) - entsize * spec[1] not in (2, 3):
        printh("expected struct spec : %d = %d" % (spec[1], (len(spec) - 2) // 5))
...
def dumpenum(id0, node):
    ...
    # flags>>3 -> width
    # flags&1 -> bitfield
    printh("enum %s, 0x%x, 0x%x, 0x%x" % (name, size, display, flags))
...
def printent(args, id0, c):
    if args.verbose:
        printh("%s = %s" % (id0.prettykey(c.getkey()), id0.prettyval(c.getval())))
    else:
        printh("%s = %s" % (hexdump(c.getkey()), hexdump(c.getval())))
...
def createkey(args, id0, base, tag, ix):
    ...
    if re.match(r'^#(?:0[xX][0-9a-fA-F]+|\d+)$', base):
        ...
        if nodeid and args.verbose > 1:
            printh("found node %x for %s" % (nodeid, base))
    if nodeid is None:
        printh("Could not find '%s'" % base)
        return
...
def processidb(args, idb):
    if args.verbose > 1:
        printh("magic=%s, filever=%d" % (idb.magic, idb.fileversion))
        for i in range(6):
            comp, ofs, size, checksum = idb.getsectioninfo(i)
            if ofs:
                part = idb.getpart(i)
                printh("%2d: %02x, %08x %8x [%08x]:  %s" % (i, comp, ofs, size, checksum, hexdump(part.read(256))))
    ...
    try:
        ...
        if magic.startswith(b"Va") or magic.startswith(b"VA"):
            ...
            if filetypehint == 'id1':
                ...
                printh("unknown VA type file: %s" % hexdump(magic))
...
def DirEnumerator(args, path):
    ...
    for d in os.scandir(path):
        try:
            ...
            printh("EXCEPTION %s accessing %s/%s" % (e, path, d.name))
...
def EnumeratePaths(args, paths):
    ...
    for fn in paths:
        try:
            ...
            printh("EXCEPTION %s accessing %s" % (e, fn))
...
""")
    parser.set_defaults(default=sys.stderr)
    ...
    if args.FILES:
        ...
        for fn in EnumeratePaths(args, args.FILES):
            ...
            if isv2name(filename):
                ...
                printh("%s -> %s : %s" % (xlatv2name(filename), basepath, filename))
            else:
        ...
        # processfile(args, args.filetype, sys.stdin)

if __name__ == '__main__':
    global printh_fh
    printh_fh = sys.stderr
    path = "C:\\Users\\alzakariyamq\\Dev\\GameDev\\Mods\\GBA\\MMBN6\\mmbn6g.idb"
    query = "?0x800ebd4"
    main(('%s -v -q %s' % (path, query)).split(' '))
